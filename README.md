...
- **TheFalloftheOverlord**（剧情外壳）：三幕剧情、场景、对话、地图、逐字打印。
...
- 主线「打斗」（第二幕王翳/追兵、第三幕秦时月与最终决战）通过 `story.json` 的 `battle`/`battleId` 字段接入 `CombatSystem`。
- 两幕网格地图（垓下营地 / 淮河）由 `MapLayouts` 摆布局、`SceneMap` 跑交互，支持**传送门地图链**（垓下↔营外荒郊、淮河↔阴陵一/二/三）；第三幕为纯剧情终章（东城快战与乌江自刎）。
...
│   ├── story/                # 剧情文案
│   │   ├── story.json        # 三幕剧情（旁白/对话/分支/战斗触发/幕次旁白 advance）
...
│       ├── MapLayouts.h/.cpp        # 两幕主线地图 + 支线子地图布局
...
- **剧情**：三幕（垓下 / 淮河 / 东城·乌江），从 `story.json` 加载；逐字打印、自动/手动播放（ESC 切换）、按 q 加速、第二幕分支选择、幕次跳转旁白（`advance` 字段）。
...
- `data/story/story.json`：三幕剧情。每幕含 `advance`（幕次跳转旁白）与 `lines`（`text`/`color`/`sleep`/`wait`/`battle`/`battleId`），第二幕含 `choice`（`prompt` + `options`，每项含 `key`/`branch`/`lines`）。

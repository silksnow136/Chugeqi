...
void MapGrid::buildPortal(int r, int c, PortalDir dir, const std::string& dest, int length) {
    ...
    // 文字提示「去<dest><箭头>」，dest 存完整名称（供传送目标查找），显示宽度按格自动换行
    std::string text = "去" + dest + portalArrow(dir);
    ...
    for (int i = 0; i < len; i++) {
        ...
        Tile t(padToWidth(disp, CELL_WIDTH), TileType::PORTAL, dest, static_cast<int>(dir));
        grid[r][cc] = t;
    }
...
void MapGrid::move(char direction) {
    ...
    // 拷贝目标格，避免下方改引用造成的别名问题
    Tile target = grid[newRow][newCol];
    if (target.type == TileType::WALL) return;

    // 可通行（空地/门/物品）：移动玩家；物品踩上后消失并触发拾取
    if (target.type == TileType::EMPTY || target.type == TileType::DOOR || target.type == TileType::ITEM) {
        grid[playerRow][playerCol] = underPlayer;
        underPlayer = (target.type == TileType::ITEM) ? Tile(EMPTY_DISP, TileType::EMPTY) : target;
        playerRow = newRow;
        playerCol = newCol;
